...
class ViewController: UICollectionViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, MCSessionDelegate, MCBrowserViewControllerDelegate {
    ...
    // var peerID: MCPeerID?  // Removed to get rid of optional
...
//        navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .camera, target: self, action: #selector(importPicture))
        ...
        // peerID = MCPeerID(displayName: UIDevice.current.name) // removed to get rid of optiona, assigned during initialization
        mcSession = MCSession(peer: peerID, securityIdentity: nil, encryptionPreference: .required)  // error occurs because peerID needs to be NON optional
        mcSession?.delegate = self  // need to conform right protocol to remove error
    }
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        switch state {
        case .connected:
            let ac = UIAlertController(title: "New connection!", message: "\(peerID.displayName) has connected!", preferredStyle: .alert)
