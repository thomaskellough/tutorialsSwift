...
class ImageViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        guard let imageTitle = image?.replacingOccurrences(of: "-Large.jpg", with: "") else { return }
        title = imageTitle
	...
	override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let key = image else { return }
        let defaults = UserDefaults.standard
		var currentVal = defaults.integer(forKey: key)
		currentVal += 1

		defaults.set(currentVal, forKey:key)
