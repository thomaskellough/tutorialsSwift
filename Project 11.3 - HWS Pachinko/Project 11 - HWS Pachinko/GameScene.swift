...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        background.position = CGPoint(x: 512, y: 384)  // Remeber: 0,0 is CENTER, unlike UIKit where it's top left
        background.blendMode = .replace
        background.zPosition = -1  // Puts image behind?
        addChild(background)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let location = touch.location(in: self)  // Find where the touch was in the entire game scene
            ...
            if editingMode{
                ...
                if ballCount > 0 {
                    if location.y >= 600 {
                        ...
                        let size = CGFloat(ball.size.width / 2.0)
                        ball.physicsBody = SKPhysicsBody(circleOfRadius: size)
                        ball.physicsBody?.restitution = 0.4  // Refers to bounciness (0: not bouncy, 1: super bouncy)
    ...
    func makeBouncer(at position: CGPoint) {
        ...
        bouncer.physicsBody?.isDynamic = false  // Will not move, can collide, but won't move, fixed in place
        addChild(bouncer)
    }
